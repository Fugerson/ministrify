...
@section('title', __('Посилання застаріло'))
    ...
    <div class="max-w-md w-full space-y-8">
        <div class="text-center">
            ...
            <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white">
                {{ __('Посилання застаріло') }}
            </h2>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Посилання для підтвердження email більше не дійсне') }}
            </p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
            <div class="text-center space-y-4">
                <p class="text-gray-600 dark:text-gray-300">
                    {{ __('Посилання для верифікації дійсне протягом 24 годин. Увійдіть до свого акаунту, щоб отримати нове посилання.') }}
                </p>
                ...
                <a href="{{ route('login') }}"
                   class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                    {{ __('Увійти до акаунту') }}
                </a>

                <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{ __('Після входу ви зможете надіслати новий лист для підтвердження') }}
                </p>
            </div>
        </div>
    </div>
