...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('app.two_factor_auth') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('app.extra_account_protection') }}
            </p>
        </div>
        ...
        <div class="p-6">
            @if($enabled)
                ...
                <div class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl p-4 mb-6">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-green-800 dark:text-green-300">{{ __('app.2fa_enabled') }}</h3>
                            <p class="mt-1 text-sm text-green-700 dark:text-green-400">
                                {{ __('app.account_protected_by_2fa') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                @if(count($recoveryCodes) > 0)
                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">{{ __('app.recovery_codes') }}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">
                            {{ __('app.save_codes_in_safe_place') }}
                        </p>
                    ...
                    <form action="{{ route('two-factor.regenerate') }}" method="POST" class="inline">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {{ __('app.password_for_confirmation') }}
                            </label>
            ...
            @else
                ...
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('app.2fa_not_enabled') }}</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                        {{ __('app.enable_2fa_description') }}
                    </p>
