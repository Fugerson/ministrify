...
@section('title', 'Двофакторна аутентифікація')
        ...
        <div class="p-6">
            @if($enabled)
                ...
                <div class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl p-4 mb-6">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-green-800 dark:text-green-300">2FA увімкнено</h3>
                            <p class="mt-1 text-sm text-green-700 dark:text-green-400">
                                Ваш обліковий запис захищений двофакторною аутентифікацією.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                @if(count($recoveryCodes) > 0)
                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Коди відновлення</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">
                            Збережіть ці коди в безпечному місці. Вони знадобляться, якщо втратите доступ до телефону.
                        </p>
                ...
                <div class="space-y-4">
                    <form action="{{ route('two-factor.regenerate') }}" method="POST" class="inline">
                        ...
                        <button type="submit"
                            ...
                            Згенерувати нові коди відновлення
                        </button>
                    </form>

                    <form action="{{ route('two-factor.disable') }}" method="POST"
                          onsubmit="return confirm('Ви впевнені, що хочете вимкнути 2FA?')">
                        @csrf
                        @method('DELETE')
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Пароль для вимкнення
                            </label>
                            <input type="password" name="password" required
                                   class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg">
                        </div>
            ...
            @else
                ...
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">2FA не увімкнено</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                        Увімкніть двофакторну аутентифікацію для додаткового захисту вашого облікового запису.
                    </p>
