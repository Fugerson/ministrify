...
@section('title', __('auth.complete_registration_title'))
...
<div class="text-center mb-6">
    ...
    <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('auth.hello_name', ['name' => $googleUser['name']]) }}</h2>
...
<form method="POST" action="{{ route('auth.google.complete') }}" class="space-y-5" x-data="{ action: 'create_church' }">
    ...
    <div class="space-y-3">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('auth.what_do_you_want') }}</label>
        ...
        <label class="flex items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-xl cursor-pointer border-2 transition-all"
            ...
            <div>
                <div class="font-medium text-gray-900 dark:text-white">{{ __('auth.create_new_church') }}</div>
                <div class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.i_am_admin') }}</div>
            </div>
        </label>
        ...
        <label class="flex items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-xl cursor-pointer border-2 transition-all"
            ...
            <div>
                <div class="font-medium text-gray-900 dark:text-white">{{ __('auth.join_church') }}</div>
                <div class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.my_church_in_system') }}</div>
            </div>
        </label>
    </div>
    ...
    <div x-show="action === 'create_church'" x-cloak class="space-y-4">
        <div>
            <label for="church_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('auth.church_name') }}</label>
            <input type="text" name="church_name" id="church_name" value="{{ old('church_name') }}"
                   ...
                   placeholder="{{ __('auth.church_placeholder') }}">
            @error('church_name')
        ...
        <div>
            <label for="city" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('auth.city') }}</label>
    ...
    <div x-show="action === 'join_church'" x-cloak class="space-y-4">
        <div>
            <label for="church_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('auth.select_church') }}</label>
            <select name="church_id" id="church_id"
                    class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 dark:text-white border-0 rounded-xl focus:bg-white dark:focus:bg-gray-600 focus:ring-2 focus:ring-primary-500/20 transition-all">
                <option value="">-- {{ __('auth.select_church') }} --</option>
            ...
            @enderror
            @if($churches->isEmpty())
                <p class="mt-2 text-sm text-amber-600 dark:text-amber-400">{{ __('auth.no_churches_available') }}</p>
            @endif
        </div>
    </div>
