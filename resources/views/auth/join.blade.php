...
<form method="POST" action="{{ route('join.store') }}" class="space-y-5" x-data="{ showPassword: false }">
    ...
    <a href="#" @click.prevent="if(!document.getElementById('church_id').value) { alert('{{ __('auth.first_select_church') }}'); return; } window.location.href='{{ route('auth.google') }}?church_id=' + document.getElementById('church_id').value"
