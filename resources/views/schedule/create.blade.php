    ...
    <form method="POST" action="{{ route('events.store') }}" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
            ...
            <div class="space-y-4">
                ...
                <!-- Service Plan Option -->
                <div class="pt-4 border-t border-gray-200 dark:border-gray-600" x-data="{ isService: {{ old('is_service') ? 'true' : 'false' }} }">
                    ...
                    <p class="mt-1 ml-6 text-xs text-gray-500 dark:text-gray-400">
                        Увімкніть, щоб створити план служіння з таймлайном (прославлення, проповідь, оголошення тощо)
                    </p>
                    ...
                    <div x-show="isService" x-transition class="mt-3">
                        ...
                        <select name="service_type" id="service_type"
                            ...
                            @foreach(\App\Models\Event::serviceTypeLabels() as $type => $label)
                                <option value="{{ $type }}" {{ old('service_type') == $type ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
