            ...
            <div class="px-4 py-3 flex items-center justify-between gap-4" x-data="{ showRoleSelect: false, selectedRole: '' }">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <div x-show="showRoleSelect" x-cloak class="flex items-center gap-2">
                        ...
                        <button @click="if(!selectedRole) return;
                            ajaxAction('{{ route('settings.users.update', $user) }}', 'PUT', {
                                name: @js($user->name),
                                email: @js($user->email),
                    ...
                    <div x-show="!showRoleSelect" class="flex items-center gap-2">
                        ...
                        <button @click="ajaxDelete('{{ route('settings.users.destroy', $user) }}', '{{ __('messages.confirm_reject_request_from', ['name' => addslashes($user->name)]) }}', () => location.reload())"
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr x-show="showRow({{ $loop->index }})" x-cloak>
                        ...
                        <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <button @click="ajaxDelete('{{ route('settings.users.destroy', $user) }}', '{{ __('messages.are_you_sure') }}', () => location.reload())"
