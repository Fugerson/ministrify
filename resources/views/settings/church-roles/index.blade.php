...
@section('title', __('app.church_roles_title'))

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="churchRolesManager()">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('app.church_roles_title') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('app.church_roles_subtitle') }}</p>
        </div>
            ...
            <template x-for="role in roles" :key="role.id">
                <div class="flex items-center gap-4 p-4 group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                        <span x-text="role.people_count"></span> {{ __('app.people_count_suffix') }}
                    </div>
                    ...
                    <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button @click="toggleAdmin(role.id)"
                                ...
                                :title="role.is_admin_role ? @js(__('app.remove_full_access')) : @js(__('app.grant_full_access'))">
                        ...
                        <template x-if="!role.is_admin_role">
                            <button @click="openPermissions(role.id, role.name)"
                                    ...
                                    title="{{ __('app.configure_permissions') }}">
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-700/50">
            <div class="flex items-center gap-4">
                ...
                <input type="text" x-model="newName"
                       placeholder="{{ __('app.new_role_placeholder') }}"
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
        <div class="flex gap-3">
            ...
            <div class="text-sm text-blue-800 dark:text-blue-200">
                <p class="font-medium">{{ __('app.hints_title') }}</p>
                <ul class="mt-1 list-disc list-inside space-y-1 text-blue-700 dark:text-blue-300">
                    <li>{{ __('app.hint_drag_roles') }}</li>
                    <li>{{ __('app.hint_double_click') }}</li>
                    <li>{{ __('app.hint_default_role') }}</li>
                </ul>
            </div>
        </div>
    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-2xl w-full p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ __('app.permissions_for') }} <span x-text="permissionsRoleName"></span>
                        </h3>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="text-left border-b border-gray-200 dark:border-gray-700">
                                    <th class="pb-2 font-medium text-gray-900 dark:text-white">{{ __('app.module_column') }}</th>
                                    <th class="pb-2 font-medium text-gray-900 dark:text-white text-center w-20">{{ __('app.view_column') }}</th>
                                    <th class="pb-2 font-medium text-gray-900 dark:text-white text-center w-20">{{ __('app.create_short') }}</th>
                                    <th class="pb-2 font-medium text-gray-900 dark:text-white text-center w-20">{{ __('app.edit_short') }}</th>
                                    <th class="pb-2 font-medium text-gray-900 dark:text-white text-center w-20">{{ __('app.delete_short') }}</th>
                                </tr>
                            </thead>
                    ...
                    <div class="flex justify-end gap-3 mt-6">
                        ...
                        <button @click="savePermissions()"
                            ...
                            <span x-text="savingPermissions ? @js(__('app.saving')) : @js(__('app.save'))"></span>
                        </button>
                    </div>
                </div>
            </div>
...
function churchRolesManager() {
    return {
        ...
        modules: @json(\App\Models\ChurchRolePermission::getTranslatedModules()),
        ...
        async setDefault(id) {
            try {
                ...
                if (response.ok) {
                    ...
                    if (window.showGlobalToast) showGlobalToast(@js(__('app.default_updated_toast')), 'success');
                }
            } catch (error) {
