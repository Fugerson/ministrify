...
$pmUnreadCount = $churchId ? \App\Models\PrivateMessage::unreadCount($churchId, auth()->id()) : 0;
$announcementUnreadCount = $churchId ? \App\Models\Announcement::unreadCount($churchId, auth()->id()) : 0;

$commTabs = [
    ['route' => 'announcements.index', 'label' => __('app.announcement_action'), 'active' => 'announcements.*', 'icon' => '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>', 'badge' => $announcementUnreadCount],
    ['route' => 'pm.index', 'label' => __('app.chat_label'), 'active' => 'pm.*', 'icon' => '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>', 'badge' => $pmUnreadCount, 'badgeId' => 'pm-badge'],
];
if(auth()->user()->canCreate('announcements')) {
    $commTabs[] = ['route' => 'messages.index', 'label' => __('app.mailing_label'), 'active' => 'messages.*', 'icon' => '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>'];
}
@endphp
