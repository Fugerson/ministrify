...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden"
    ...
    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <h3 class="font-semibold text-gray-900 dark:text-white">{{ __('app.checklist_title') }}</h3>
        </div>
                    ...
                    <template x-if="adding">
                        <form @submit.prevent="addItem()" class="flex items-center gap-2">
                            <input type="text" x-model="newTitle" required x-ref="newItemInput"
                                   placeholder="{{ __('app.checklist_item_placeholder') }}"
    ...
    <template x-if="!hasChecklist">
        ...
        <div class="p-6">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">{{ __('app.checklist_add_desc') }}</p>
                    ...
                    @if($templates->isNotEmpty())
                        <div class="relative">
                            <button type="button" @click="showTemplates = !showTemplates"
                                ...
                                {{ __('app.checklist_from_template') }}
                            </button>
                            ...
                            <div x-show="showTemplates" x-cloak @click.away="showTemplates = false"
                                ...
                                @foreach($templates as $template)
                                    <button type="button" @click="createChecklist({{ $template->id }})"
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('app.checklist_items_count', ['count' => $template->items->count()]) }}</p>
                                    </button>
                                @endforeach
                            </div>
                        </div>
    ...
    return {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        ...
                        this.updateProgress();
                        if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_created') ), 'success');
                    }
                }
            } catch (err) {
                console.error(err);
                if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_create_error') ), 'error');
            }
        },
        ...
        async deleteChecklist() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.items = [];
                    this.progress = 0;
                    if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_deleted') ), 'success');
                }
            } catch (err) {
                console.error(err);
                if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_deletion_error') ), 'error');
            }
        },
        ...
        async toggleItem(item) {
            try {
                ...
                console.error(err);
                if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_error') ), 'error');
            }
        },
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success && data.item) {
                        ...
                        this.adding = false;
                        this.updateProgress();
                        if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_item_added') ), 'success');
                    }
                }
            } catch (err) {
                console.error(err);
                if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_item_add_error') ), 'error');
            }
        },
        ...
        async deleteItem(item) {
            try {
                ...
                if (response.ok) {
                    ...
                    this.updateProgress();
                    if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_item_deleted') ), 'success');
                }
            } catch (err) {
                console.error(err);
                if (window.showGlobalToast) showGlobalToast(@js( __('app.checklist_deletion_error') ), 'error');
