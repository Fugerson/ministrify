@props(['entityType', 'entityId', 'boards' => [], 'defaultBoard' => null])
...
<div x-data="linkedCards()" x-init="loadCards()" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
    <div class="flex items-center justify-between mb-4">
        ...
        @if($defaultBoard)
        <form action="{{ route('boards.create-from-entity') }}" method="POST" class="inline">
            @csrf
            <input type="hidden" name="entity_type" value="{{ $entityType }}">
            <input type="hidden" name="entity_id" value="{{ $entityId }}">
            <input type="hidden" name="board_id" value="{{ $defaultBoard->id }}">
            <button type="submit" class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
