...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Середня відвідуваність</h2>

        <div class="text-center">
            <p class="text-5xl font-bold text-primary-600">{{ round($monthlyAttendance ?? 0) }}</p>
            <p class="text-gray-500">осіб в середньому</p>
        </div>
    </div>

    <!-- Chart -->
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Динаміка за останні 12 тижнів</h2>

        <div class="h-64 flex items-end justify-between space-x-2">
            @php $maxCount = max(array_column($chartData, 'count')) ?: 1; @endphp
            @foreach($chartData as $data)
                <div class="flex-1 flex flex-col items-center">
                    <div class="w-full bg-primary-100 rounded-t"
                ...
                <div class="px-6 py-4 flex items-center justify-between">
                    <div class="flex items-center">
                        <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center">
                            <span class="text-gray-600">{{ substr($person->first_name, 0, 1) }}</span>
                        </div>
                        <div class="ml-3">
                            <a href="{{ route('people.show', $person) }}" class="font-medium text-gray-900 hover:text-primary-600">
                                {{ $person->full_name }}
                            </a>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
                        ...
                        @if($person->telegram_username)
                            <span class="text-gray-500 text-sm">{{ $person->telegram_username }}</span>
                        @endif
                    </div>
                </div>
            @endforeach
    ...
    <a href="{{ route('attendance.index') }}" class="inline-block text-gray-600 hover:text-gray-900">
