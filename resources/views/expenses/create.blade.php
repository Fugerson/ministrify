...
@section('title', __('Додати витрату'))
...
<div class="max-w-2xl mx-auto">
    ...
    <form @submit.prevent="submit($refs.expenseForm)" x-ref="expenseForm"
        ...
        @if(session('budget_exceeded'))
            <div class="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h3 class="text-sm font-medium text-orange-800 dark:text-orange-200">{{ __('Перевищення бюджету') }}</h3>
                        ...
                        <label class="mt-3 flex items-center">
                            ...
                            <span class="ml-2 text-sm text-orange-800 dark:text-orange-200">{{ __('Все одно додати витрату (потрібне підтвердження)') }}</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Нова витрата') }}</h2>

            <div class="space-y-4">
                <div>
                    <label for="ministry_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Команда') }} <span class="text-red-500">*</span></label>
                    <select name="ministry_id" id="ministry_id" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="">{{ __('Виберіть команду') }}</option>
                        @foreach($ministries as $ministry)
                            <option value="{{ $ministry->id }}" {{ old('ministry_id') == $ministry->id ? 'selected' : '' }}>
                                {{ $ministry->name }}
                                @if($ministry->monthly_budget)
                                    ({{ __('залишок') }}: {{ number_format($ministry->remaining_budget, 0, ',', ' ') }} ₴)
                                @endif
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description') }}" required
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           placeholder="{{ __('Струни для гітари') }}">
                    <template x-if="errors.description"><p class="mt-1 text-sm text-red-500" x-text="errors.description[0]"></p></template>
                </div>

                <div>
                    <label for="category_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Категорія') }}</label>
                    <select name="category_id" id="category_id"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="">{{ __('Без категорії') }}</option>
                ...
                <div>
                    <label for="receipt_photo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Фото чека') }}</label>
                    <div x-data="{ fileName: '' }" class="relative">
                        ...
                        <label @click="$refs.receiptInput.click()" class="flex items-center gap-3 px-4 py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all group">
                            ...
                            <div class="flex-1 min-w-0">
                                <p x-show="!fileName" class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Обрати фото чека') }}</p>
