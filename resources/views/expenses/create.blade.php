...
@section('title', __('app.expense_add_title'))

@section('content')
<div class="max-w-2xl mx-auto">
    <a href="{{ route('finances.transactions', ['filter' => 'expense']) }}" class="inline-flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-sm mb-6">
    ...
    <form @submit.prevent="submit($refs.expenseForm)" x-ref="expenseForm"
          x-data="{ ...ajaxForm({ url: '{{ route('finances.expenses.store') }}', method: 'POST' }) }"
        ...
        @if(session('budget_exceeded'))
            <div class="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h3 class="text-sm font-medium text-orange-800 dark:text-orange-200">{{ __('app.expense_budget_exceeded') }}</h3>
                        ...
                        <label class="mt-3 flex items-center">
                            ...
                            <span class="ml-2 text-sm text-orange-800 dark:text-orange-200">{{ __('app.expense_force_add') }}</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('app.expense_new') }}</h2>

            <div class="space-y-4">
                <div>
                    <label for="ministry_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_team') }} <span class="text-red-500">*</span></label>
                    <select name="ministry_id" id="ministry_id" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="">{{ __('app.expense_select_team') }}</option>
                        @foreach($ministries as $ministry)
                            <option value="{{ $ministry->id }}" {{ old('ministry_id') == $ministry->id ? 'selected' : '' }}>
                                {{ $ministry->name }}
                                @if($ministry->monthly_budget)
                                    ({{ __('app.expense_remaining') }}: {{ number_format($ministry->remaining_budget, 0, ',', ' ') }} ₴)
                                @endif
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_amount') }} <span class="text-red-500">*</span></label>
                        <div class="relative">
                    ...
                    <div>
                        <label for="date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_date') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_description') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="category_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_category') }}</label>
                    <select name="category_id" id="category_id"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="">{{ __('app.expense_no_category') }}</option>
                ...
                <div>
                    <label for="receipt_photo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_receipt_photo') }}</label>
                    <div x-data="{ fileName: '' }" class="relative">
                        <input type="file" name="receipt_photo" id="receipt_photo" accept="image/*,.heic,.heif" class="sr-only" x-ref="receiptInput" @change="fileName = $event.target.files[0]?.name || ''">
                        <label @click="$refs.receiptInput.click()" class="flex items-center gap-3 px-4 py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all group">
                            ...
                            <div class="flex-1 min-w-0">
                                <p x-show="!fileName" class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.expense_select_receipt') }}</p>
                ...
                <div>
                    <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.expense_notes') }}</label>
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2 sm:gap-3">
            ...
            <button type="submit" :disabled="saving" class="w-full sm:w-auto px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50">
                <span x-show="!saving">{{ __('app.expense_save') }}</span>

                <span x-show="saving">{{ __('app.expense_saving') }}</span>
            </button>
        </div>
    </form>
</div>
