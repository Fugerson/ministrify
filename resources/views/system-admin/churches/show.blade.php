...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-start gap-6">
            ...
            @else
            <div class="w-24 h-24 rounded-xl bg-blue-100 dark:bg-blue-600/20 flex items-center justify-center">
                <svg class="w-12 h-12 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
            </div>
            @endif
            <div class="flex-1">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->name }}</h2>
                <p class="text-gray-500 dark:text-gray-400 mt-1">{{ $church->city }}</p>
                @if($church->address)
                <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">{{ $church->address }}</p>
                @endif
                <div class="flex items-center gap-4 mt-4">
                    <span class="px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full text-sm">ID: {{ $church->id }}</span>
                    <span class="px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full text-sm">Slug: {{ $church->slug }}</span>
                    @if($church->public_site_enabled)
                    <span class="px-3 py-1 bg-green-100 dark:bg-green-600/20 text-green-700 dark:text-green-400 rounded-full text-sm">Публічний сайт</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->people_count }}</p>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Людей</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->ministries_count }}</p>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Служінь</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->groups_count }}</p>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Груп</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->events_count }}</p>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Подій</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $church->boards_count }}</p>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Дошок</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="font-semibold text-gray-900 dark:text-white">Користувачі церкви</h2>
            </div>
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <div class="flex items-center px-6 py-3">
                    <div class="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center mr-3">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ mb_substr($user->name, 0, 1) }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        ">{{ $user->role }}</span>
                        <a href="{{ route('system.users.edit', $user) }}" class="p-1 text-gray-400 hover:text-gray-900 dark:hover:text-white">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="font-semibold text-gray-900 dark:text-white">Останні події</h2>
            </div>
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                @empty
                <p class="px-6 py-4 text-gray-500 dark:text-gray-400">Немає подій</p>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="font-semibold text-gray-900 dark:text-white">Останні дії</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">Немає записів</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
