...
<div class="max-w-2xl">
    ...
    <div class="bg-gray-800 rounded-xl p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-bold text-white">Редагувати задачу</h2>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                <span>Створено: {{ $task->created_at->format('d.m.Y H:i') }}</span>
                @if($task->completed_at)
                    <span class="text-green-400">| Виконано: {{ $task->completed_at->format('d.m.Y H:i') }}</span>
                @endif
            </div>
        </div>
        ...
        <form action="{{ route('system.tasks.update', $task) }}" method="POST" class="space-y-6">
            ...
            @if($task->supportTicket)
                <div class="p-4 bg-gray-700/50 rounded-lg">
                    ...
                    <a href="{{ route('system.support.show', $task->supportTicket) }}" class="text-red-400 hover:text-red-300 font-medium">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-2">Пов'язаний тікет</label>
            ...
            <div class="flex items-center justify-between pt-4">
                ...
                <form action="{{ route('system.tasks.destroy', $task) }}" method="POST" class="inline" onsubmit="return confirm('Видалити задачу?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 text-red-400 hover:text-red-300 transition-colors">
                        Видалити
                    </button>
                </form>
            </div>
