...
@section('actions')
<a href="{{ route('system.tasks.create') }}" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-colors">
    + Нова задача
</a>
@endsection
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-gray-800 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-400 text-sm">Беклог</p>
                    <p class="text-2xl font-bold text-white">{{ $stats['backlog'] }}</p>
                </div>
        ...
        <div class="bg-gray-800 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-400 text-sm">До виконання</p>
        ...
        <div class="bg-gray-800 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-400 text-sm">В роботі</p>
        ...
        <div class="bg-gray-800 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-400 text-sm">Тестування</p>
        ...
        <div class="bg-gray-800 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-400 text-sm">Виконано</p>
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        <form action="{{ route('system.tasks.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <select name="status" class="px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-red-500">
            ...
            <select name="type" class="px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-red-500">
            ...
            <select name="priority" class="px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-red-500">
    ...
    <div class="bg-gray-800 rounded-xl overflow-hidden">
        @if($tasks->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-gray-400">Немає задач</p>
                <a href="{{ route('system.tasks.create') }}" class="inline-block mt-4 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">
        ...
        @else
            <table class="w-full">
                <thead class="bg-gray-700/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Задача</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Тип</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Пріоритет</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Статус</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Виконавець</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Дії</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-700/50">
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    @if($task->supportTicket)
                                        <a href="{{ route('system.support.show', $task->supportTicket) }}" class="text-sm text-red-400 hover:text-red-300">
                                            Тікет #{{ $task->supportTicket->id }}
                                        </a>
                                    @endif
                                    @if($task->due_date)
                                        <p class="text-sm {{ $task->due_date->isPast() && $task->status !== 'done' ? 'text-red-400' : 'text-gray-400' }}">
                            ...
                            <td class="px-6 py-4">
                                <form action="{{ route('system.tasks.update-status', $task) }}" method="POST" class="inline">
                                    ...
                                    <select name="status" onchange="this.form.submit()"
                                            class="px-2 py-1 text-xs font-medium rounded-full border-0 bg-{{ $task->status_color }}-500/20 text-{{ $task->status_color }}-400 focus:ring-2 focus:ring-red-500 cursor-pointer">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('system.tasks.edit', $task) }}" class="p-2 text-gray-400 hover:text-white transition-colors">
