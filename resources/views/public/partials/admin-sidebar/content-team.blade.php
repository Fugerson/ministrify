...
<div class="space-y-2">
                ...
                <div class="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex gap-2 mt-0.5">
                            <button x-on:click="contentEdit('team', item, ['name', 'title', 'bio']); cnt.team.form._photoPreview = item.photo ? '/storage/' + item.photo : null; cnt.team.form.photo = null;" class="text-[10px] text-blue-500 hover:text-blue-700">{{ __('app.sb_edit_btn') }}</button>
                            <button x-on:click="contentDeleteItem('team', item.id)" class="text-[10px] text-red-500 hover:text-red-700">{{ __('app.sb_delete_btn') }}</button>
                        </div>
                    </div>
                </div>
            </template>
        </div>
        <p x-show="cnt.team.loaded && cnt.team.items.length === 0" class="text-xs text-gray-400 text-center py-2">{{ __('app.sb_no_team_members') }}</p>
    </div>
    ...
    <div x-show="cnt.team.editing !== null" class="space-y-2">
        ...
        <input x-model="cnt.team.form.name" type="text" placeholder="{{ __('app.sb_name_ph') }}"
        ...
        <input x-model="cnt.team.form.title" type="text" placeholder="{{ __('app.sb_position_ph') }}"
               class="w-full text-xs border border-gray-300 rounded-lg px-2.5 py-1.5 focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
        <textarea x-model="cnt.team.form.bio" placeholder="{{ __('app.sb_bio_ph') }}" rows="2"
