...
@section('description', ($group->public_description ?? $group->description ?? $group->name) . ' — ' . $church->name)
@if($group->cover_image)
    @section('og_image', Storage::url($group->cover_image))
@endif
@section('breadcrumbs')
{!! json_encode([
        ...
        <div class="p-8 md:p-10">
            ...
            <div class="flex flex-wrap gap-4 mb-6">
                ...
                <div class="flex items-center gap-2 text-gray-600">
                    ...
                    <span>{{ __('app.public_ministry_members_count', ['count' => $group->members_count]) }}</span>
                </div>
            </div>
            ...
            @if($group->leader)
                <div class="flex items-center gap-3 p-4 bg-gray-50 rounded-xl mb-6">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">{{ __('app.group_leader') }}</p>
                ...
                <div class="border-t border-gray-100 pt-8" x-data="{ showForm: false }">
                    <div class="bg-primary-50 rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <h3 class="font-semibold text-gray-900">{{ __('app.public_group_join') }}</h3>
                                <p class="text-sm text-gray-600">{{ __('app.public_group_join_desc') }}</p>
                            </div>
                            ...
                            <form action="{{ route('public.group.join', [$church->slug, $group->slug]) }}" method="POST" class="space-y-4"
                                ...
                                <div>
                                    ...
                                    <textarea name="message" rows="3"
                                              class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="{{ __('app.public_group_tell_about_yourself') }}">{{ old('message') }}</textarea>
                                </div>
