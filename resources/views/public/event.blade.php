...
{!! json_encode([
    ...
    'itemListElement' => [
        ...
        ['@type' => 'ListItem', 'position' => 2, 'name' => __('app.events'), 'item' => route('public.events', $church->slug)],
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <nav class="mb-8">
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li><a href="{{ route('public.events', $church->slug) }}" class="text-gray-500 hover:text-primary-600">{{ __('app.events') }}</a></li>
            <li class="text-gray-400">/</li>
                    ...
                    @if($event->canAcceptRegistrations())
                        <div class="bg-primary-50 rounded-2xl p-6 mb-6">
                            <div class="flex items-center justify-between mb-4">
                                <div>
                                    <h3 class="font-semibold text-gray-900">{{ __('app.public_event_registration') }}</h3>
                                    @if($event->registration_limit)
                                        <p class="text-sm text-gray-600">{{ __('app.public_event_spaces_remaining', ['spaces' => $event->remaining_spaces, 'limit' => $event->registration_limit]) }}</p>
                                    @endif
                                    @if($event->registration_deadline)
                                        <p class="text-sm text-gray-600">{{ __('app.public_event_registration_deadline', ['date' => $event->registration_deadline->format('d.m.Y H:i')]) }}</p>
                                    @endif
                                </div>
                                <button @click="showForm = !showForm"
                                        class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-xl transition-colors">
                                    <span x-show="!showForm">{{ __('app.public_event_register_btn') }}</span>
                            ...
                            <div x-show="showForm" x-cloak x-transition class="mt-6">
                                <form action="{{ route('public.event.register', [$church->slug, $event]) }}" method="POST" class="space-y-4"
                                      ...
                                      x-on:submit.prevent="
                                          ...
                                          if (typeof grecaptcha !== 'undefined') {
                                              grecaptcha.ready(function() {
                                                  grecaptcha.execute('{{ config('services.recaptcha.site_key') }}', {action: 'event_register'}).then(function(token) {
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.public_event_guests_count') }}</label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.comment_placeholder') }}</label>
    ...
    <div class="mt-8 text-center">
        <a href="{{ route('public.events', $church->slug) }}" class="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 font-medium">
            ...
            {{ __('app.public_event_back_to_list') }}
        </a>
    </div>
</div>
@endsection
