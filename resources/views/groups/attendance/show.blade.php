...
<div class="flex items-center gap-2">
    ...
    <button type="button"
            ...
            class="inline-flex items-center px-4 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm font-medium rounded-xl hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors">
        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            @if($record->person)
            @php
                $pivotRole = $group->members->firstWhere('id', $record->person_id)?->pivot?->role;
            @endphp
                ...
                <div class="flex items-center">
                    ...
                    <div class="mr-3" x-data="{ hover: false, r: {} }" @mouseenter="hover = true; r = $el.getBoundingClientRect()" @mouseleave="hover = false">
                        ...
                        <div class="fixed z-[100] pointer-events-none" :style="`left:${r.left+r.width/2}px;top:${r.top-8}px;transform:translate(-50%,-100%)`">
                            <img src="{{ Storage::url($record->person->photo) }}" :class="hover ? 'opacity-100 scale-100' : 'opacity-0 scale-75'" class="w-32 h-32 rounded-xl object-cover shadow-xl ring-2 ring-white dark:ring-gray-800 transition-all duration-200 ease-out origin-bottom">
                        </div>
                    </div>
                    @else
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('app.group_checkin_at') }} {{ $record->checked_in_at->format('H:i') }}</span>
                            @endif
                        </div>
                    </div>
                </div>
