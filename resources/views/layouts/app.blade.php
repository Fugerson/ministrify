...
<head>
    ...
    <style>
        ...
        /* Hide number input spinners (currency fields) */
    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-screen flex max-w-[100vw] overflow-x-clip menu-position-wrapper"
        ...
        <aside x-data="{ collapsed: localStorage.getItem('sidebar_collapsed') === 'true' }"
            ...
            <div class="flex items-center h-16 border-b border-gray-200 dark:border-gray-700 flex-shrink-0" :class="collapsed ? 'justify-center px-2' : 'px-6'">
                <a wire:navigate href="{{ route('dashboard') }}" class="flex items-center flex-shrink-0 min-w-0" :class="collapsed ? '' : 'space-x-2 overflow-hidden'">
                    ...
                    @else
                    <svg class="w-7 h-7 flex-shrink-0 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    @endif
        ...
        <aside x-show="sidebarOpen" x-cloak
            ...
            <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
                ...
                <a wire:navigate href="{{ route('schedule', ['tab' => 'planning']) }}" @click="sidebarOpen = false" class="flex items-center px-4 py-2.5 pl-[3.25rem] text-sm font-medium rounded-xl {{ (request()->routeIs('schedule') && request()->get('tab') === 'planning') || request()->routeIs('service-planning') ? 'bg-primary-50 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                    {{ __('app.service_planning') }}
                </a>
                @endif
                @endhasChurchRole
        ...
        <!-- Main Content -->
        <div id="main-content" class="main-content-area flex-1 min-w-0 {{ $menuPosition === 'bottom' ? 'pb-16' : ($menuPosition === 'top' ? 'lg:pt-16 pb-0' : 'pb-0') }} {{ $menuPosition === 'bottom' ? 'lg:pb-20' : 'lg:pb-0' }}"
...
<div x-data="adminScreenshot()" x-cloak @paste.window="handlePaste($event)">
    {{-- Toggle button --}}
    <button @click="open = !open" class="hidden lg:flex fixed bottom-4 left-4 z-[9999] w-12 h-12 bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-lg items-center justify-center transition-colors" title="Screenshot">
    ...
    {{-- Fullscreen preview --}}
    <div x-show="preview" x-transition.opacity class="fixed inset-0 z-[99999] bg-black/90 flex items-center justify-center cursor-pointer" @click="preview = null" @keydown.escape.window="preview = null">
        <img :src="preview" class="max-w-[95vw] max-h-[95vh] object-contain rounded-lg shadow-2xl">
    </div>
</div>
<script>
