    ...
    <div class="min-h-screen flex">
        ...
        <aside class="fixed lg:static inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col z-50 transform transition-transform duration-300 lg:translate-x-0"
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('system.activity-log') }}" @click="sidebarOpen = false"
                   class="flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('system.activity-log') ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('system.telegram-log') }}" @click="sidebarOpen = false"
                   class="flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('system.telegram-log') ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('system.screenshots') }}" @click="sidebarOpen = false"
                   class="flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('system.screenshots') ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <p class="px-4 mb-2 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Моніторинг</p>

                    @php $unresolvedErrors = \App\Models\ErrorLog::unresolved()->count(); @endphp
                    <a href="{{ route('system.errors.index') }}" @click="sidebarOpen = false"
                       class="flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('system.errors.*') ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
                        </svg>
                        Error Tracker
                        @if($unresolvedErrors > 0)
                            <span class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $unresolvedErrors }}</span>
                        @endif
                    </a>

                    <a href="/horizon" @click="sidebarOpen = false"
                       class="flex items-center px-4 py-3 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="/pulse" @click="sidebarOpen = false"
                       class="flex items-center px-4 py-3 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
