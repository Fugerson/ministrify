...
@section('title', __('app.wb_add_team_member'))
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('app.wb_add_team_member') }}</h1>
    </div>

    <form @submit.prevent="submit($refs.teamCreateForm)" x-ref="teamCreateForm" x-data="{ ...ajaxForm({ url: '{{ route('website-builder.team.store') }}', method: 'POST' }) }" class="space-y-6">

        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('app.wb_basic_info') }}</h2>
            </div>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.wb_position') }} *</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.wb_biography') }}</label>
                    <textarea name="bio" rows="4" placeholder="{{ __('app.wb_biography_placeholder') }}"
                    ...
                    <div x-data="{ fileName: '' }" class="relative">
                        ...
                        <label @click="$refs.photoInput.click()" class="flex items-center gap-3 px-4 py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all group">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">400x400px, PNG, JPG, WebP</p>
                            </div>
                        </label>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('app.wb_contacts_social') }}</h2>
            </div>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.wb_phone') }}</label>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">{{ __('app.wb_show_on_public_site') }}</span>
            </label>
        </div>
