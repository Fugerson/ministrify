                        ...
                        @if($currentTemplate !== $key)
                            <div class="flex-1">
                                <button type="button" x-data="{ loading: false }"
                                        @click="if(loading) return; loading = true; ajaxAction('{{ route('website-builder.templates.apply', $key) }}', 'POST').then(() => setTimeout(() => Livewire.navigate(window.location.href), 600)).catch(() => loading = false)"
                                        :disabled="loading"
                                        class="w-full px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50">
                                    <span x-show="!loading">{{ __('app.apply') }}</span>
