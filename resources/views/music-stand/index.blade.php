...
<div class="space-y-6">
    ...
    @if($upcomingServices->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('app.no_planned_services') }}</h3>
            <p class="text-gray-500 dark:text-gray-400">{{ __('app.when_services_appear') }}</p>
        </div>
    @else
                   ...
                   class="block bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:border-primary-300 dark:hover:border-primary-700 transition-colors">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="flex items-center text-gray-400">
                            <span class="text-sm mr-2">{{ $service->planItems->count() }} {{ __('app.songs_count_label') }}</span>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6"
         x-data="{
            search: filterStorage.load('music_stand', { search: '' }).search,
            songs: {{ json_encode($allSongs) }},
            init() {
                this.$watch('search', () => this._saveFilters());
            },
            _saveFilters() {
                filterStorage.save('music_stand', { search: this.search });
            }
         }">
        <h3 class="font-semibold text-gray-900 dark:text-white mb-4">{{ __('app.quick_song_search') }}</h3>
        <input type="text" x-model="search" placeholder="{{ __('app.enter_song_name') }}"
