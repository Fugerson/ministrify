    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6"
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {{ __('app.budget_label_colon') }} <span x-text="budgetMinistryName"></span>
            </h3>

            <form @submit.prevent="saveBudget()" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" title="{{ __('app.budget_tooltip') }}">
                        {{ __('app.monthly_budget_label') }}
                    </label>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" :disabled="budgetSaving || budgetHasItems"
                        ...
                        <span x-show="!budgetSaving">{{ __('app.save') }}</span>
                        <span x-show="budgetSaving">{{ __('app.saving') }}</span>
                    </button>
                </div>
            </form>
        </div>
