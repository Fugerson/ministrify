...
@section('title', __('app.edit_income_title'))
...
<div class="max-w-2xl mx-auto" x-data="incomeEditForm()">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('app.edit_income_title') }}</h2>
            <x-delete-confirm
                :action="route('finances.incomes.destroy', $income)"
                :redirect="route('finances.transactions', ['filter' => 'income'])"
                title="{{ __('app.delete_income_confirm') }}"
                message="{{ __('app.finance_delete_income_msg') }}"
                button-text="{{ __('app.delete') }}"
                :icon="false"
                :ajax="true"
                class="text-sm"
            />
        ...
        <form @submit.prevent="submitForm" class="p-6 space-y-6" x-ref="form">
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                          placeholder="{{ __('app.internal_notes_placeholder') }}">{{ old('notes', $income->notes) }}</textarea>
            </div>
