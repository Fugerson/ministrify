...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6"
                 x-data="{
                     ...
                     async generateCode() {
                         ...
                         try {
                             ...
                             this.error = @js(__('app.connection_error'));
                         }
                         this.loading = false;
                     },
                     copyCode() {
                ...
                @if($person->telegram_chat_id)
                    ...
                    <button type="button"
                            @click="ajaxDelete('{{ route('my-profile.telegram.unlink') }}', @js(__('app.confirm_disconnect_telegram')), () => { var s = $el.closest('.rounded-2xl'); if (s) { var inner = s.querySelector('.p-6') || s.querySelector('.px-6'); if (inner) inner.innerHTML = '\x3Cdiv class=&quot;text-center py-4&quot;>\x3Cp class=&quot;text-gray-600 dark:text-gray-400&quot;>' + @js(__('app.connect_telegram_desc')) + '\x3C/p>\x3C/div>'; } })"
                ...
                @else
                    ...
                    <div class="space-y-4">
                        ...
                        <button @click="generateCode()" :disabled="loading" type="button"
                                class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400 text-white font-medium rounded-xl transition-colors">
                            <svg x-show="loading" x-cloak class="animate-spin h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                            </svg>
                            <svg x-show="!loading" class="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.95-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.36.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .38z"/>
                            </svg>
                            <span x-text="code ? @js(__('app.get_new_code')) : @js(__('app.connect_telegram_btn'))">{{ __('app.connect_telegram_btn') }}</span>
                        </button>
                    </div>
                @endif
            </div>
                 ...
                 x-data="{
                     ...
                     async subscribe() {
                         ...
                         try {
                             ...
                             if (!publicKey) {
                                 this.error = @js( __('app.push_not_configured') );
                                 return;
                             }
                             ...
                             if (permission !== 'granted') {
                                 this.error = @js( __('app.notification_denied') );
                                 return;
                             }
                             ...
                             if (response.ok) {
                                 ...
                                 const data = await response.json().catch(() => ({}));
                                 this.error = data.error || @js( __('app.subscription_error') );
                             }
                         } catch (e) {
                             console.error('Subscribe error:', e);
                             this.error = @js( __('app.error_prefix') ) + e.message;
                         } finally {
                             this.loading = false;
                         }
                     },
                                ...
                                <template x-if="!subscribed">
                                    <div>
                                        ...
                                        <button @click="subscribe()" :disabled="loading" type="button"
                                            ...
                                            <span x-text="loading ? @js(__('app.enabling_notifications')) : @js(__('app.enable_push_btn'))"></span>
                                        </button>
                                    </div>
                                </template>
                            </div>
        ...
        <div class="space-y-6">
            ...
            {{-- Blockout Dates — hidden temporarily --}}
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        {{ __('app.language_setting') }}
                    </h2>
                </div>
                <div class="p-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">{{ __('app.language_setting_desc') }}</p>
                    <select onchange="switchLocaleAccount(this.value)"
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden"
                 x-data="{
                     ...
                     async changePassword() {
                         ...
                         try {
                             ...
                             showToast('error', '{{ __('app.error_occurred') }}');
                         }
                         this.saving = false;
                     }
                 }">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input :type="showNew ? 'text' : 'password'" x-model="new_password"
                                   class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white pr-10">
                            <button type="button" @click="showNew = !showNew" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.confirm_new_password') }}</label>
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                        class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-2xl border border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900/30 text-sm font-medium transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
