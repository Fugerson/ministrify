                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="(row, index) in filteredRows" :key="row.id || row.tempId">
                        <tr :class="{'bg-green-50 dark:bg-green-900/20': row.isNew, 'bg-amber-50 dark:bg-amber-900/20': row.isDirty && !row.isNew, 'bg-primary-50 dark:bg-primary-900/20': row.selected}"
                            ...
                            <!-- Photo -->
                            <td class="px-1 py-1">
                                <div class="relative group" x-data="{ fileInput: null, hover: false, r: {} }">
                                    <template x-if="row.photo_url">
                                        <div class="relative" @mouseenter="hover = true; r = $el.getBoundingClientRect()" @mouseleave="hover = false">
                                            ...
                                            <div class="fixed z-[100] pointer-events-none" :style="`left:${r.left+r.width/2}px;top:${r.top-8}px;transform:translate(-50%,-100%)`">
                                                <img :src="row.photo_url" :class="hover ? 'opacity-100 scale-100' : 'opacity-0 scale-75'" class="w-32 h-32 rounded-xl object-cover shadow-xl ring-2 ring-white dark:ring-gray-800 transition-all duration-200 ease-out origin-bottom">
                                            </div>
                                        </div>
                                    </template>
                                    <template x-if="!row.photo_url">
