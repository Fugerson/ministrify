...
<div x-data="peopleTable()" class="space-y-4">
        ...
        <div x-show="hasFilters" x-cloak class="flex flex-wrap gap-1.5 mt-2">
            ...
            <template x-if="filters.birth_from || filters.birth_to">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 rounded-lg text-sm">
                    <span x-text="filters.dateRangeDisplay || @js(__('app.date_of_birth'))"></span>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div x-show="!loading && people.length === 0 && hasFilters" x-cloak class="px-4 py-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('app.nothing_found') }}</h3>
                    ...
                    <div>
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-end gap-3">
                            ...
                            <button @click="executeBulkAction()" :disabled="!bulkValue || bulkLoading"
                                ...
                                <span x-show="!bulkLoading">{{ __('app.add') }}</span>
                    ...
                    <div>
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-end gap-3">
                            ...
                            <button @click="executeBulkAction()" :disabled="!bulkMessage || bulkLoading"
                                    class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors disabled:opacity-50">
                                <span x-show="!bulkLoading">{{ __('app.send') }}</span>
                                <span x-show="bulkLoading">{{ __('app.please_wait') }}</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    <p class="text-gray-900 dark:text-white">
                                        {{ __('app.confirm_delete_people') }} <strong x-text="selectedIds.length"></strong>?
                                    </p>
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-end gap-3">
                            ...
                            <button @click="executeBulkAction()" :disabled="bulkLoading"
                                ...
                                <span x-show="!bulkLoading">{{ __('app.delete') }}</span>
                                <span x-show="bulkLoading">{{ __('app.please_wait') }}</span>
                            </button>
                        </div>
                    </div>
...
<!-- Create Person Modal -->
@if(auth()->user()->canCreate('people'))
<div id="createPersonModal" class="fixed inset-0 z-50 hidden overflow-y-auto" x-data="personCreateForm()" @keydown.escape.window="closeCreatePersonModal()">
    <div class="flex items-start justify-center min-h-screen pt-4 px-4 pb-20 sm:p-0">
        <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" onclick="closeCreatePersonModal()"></div>
        <div class="relative w-full max-w-lg mx-auto mt-8 sm:mt-16 bg-white dark:bg-gray-800 rounded-2xl shadow-xl z-10">
            <form x-ref="form" @submit.prevent="submitForm">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('app.add_person') }}</h3>
                    <button type="button" onclick="closeCreatePersonModal()" class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                ...
                <div class="px-6 py-4 max-h-[70vh] overflow-y-auto space-y-5">
                    <!-- Basic Info -->
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.basic_info') }}</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.first_name') }} *</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.last_name') }} *</label>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div x-data="{ fileName: '' }" class="relative">
                                    <input type="file" name="photo" accept="image/*,.heic,.heif" class="sr-only" x-ref="modalPhotoInput" @change="fileName = $event.target.files[0]?.name || ''">
                                    <label @click="$refs.modalPhotoInput.click()" class="flex items-center gap-3 px-4 py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all group">
                    ...
                    <!-- Contacts -->
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.contacts') }}</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                    ...
                    <!-- Personal Details -->
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.details') }}</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.date_of_birth') }}</label>
                                <input type="date" name="birth_date"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.gender') }}</label>
                                <select name="gender"
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.family_status') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.anniversary') }}</label>
                                <input type="date" name="anniversary"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.joined_date') }}</label>
                            ...
                            @endphp
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.membership_status') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('app.church_role') }}</label>
                    ...
                    @if($tags->isNotEmpty())
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.tags') }}</h4>
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.ministries') }}</h4>
                        <div class="space-y-3">
                            @foreach($ministries as $ministry)
                                <div x-data="{ open: false }" class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                                    <label class="flex items-center">
                                        <input type="checkbox" name="ministries[{{ $ministry->id }}][selected]" value="1"
                                               @click="open = $event.target.checked"
                                               class="rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500">
                                        <span class="ml-2 font-medium text-gray-900 dark:text-white text-sm">{{ $ministry->name }}</span>
                                    </label>
                                    @if($ministry->positions->isNotEmpty())
                                    <div x-show="open" x-cloak class="mt-2 ml-6 space-y-1">
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('app.positions') }}:</p>
                    ...
                    <!-- Notes -->
                    <div>
                        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">{{ __('app.notes') }}</h4>
                        <textarea name="notes" rows="2"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                  placeholder="{{ __('app.additional_info') }}..."></textarea>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-2xl">
                    <button type="button" onclick="closeCreatePersonModal()"
                            class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors text-sm font-medium">
                        {{ __('app.cancel') }}
                    </button>
                    <button type="submit" :disabled="saving"
                            class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 text-sm">
            ...
            <template x-if="duplicateWarning">
                <div class="fixed inset-0 z-[60] flex items-center justify-center p-4" @keydown.escape.window.stop="duplicateWarning = null">
                ...
                if (data.success) {
                    ...
                    if (this.duplicatePairs.length === 0) {
                        setTimeout(() => {
                            ...
                            // SPA reload: merged people removed from table, counts updated
        ...
        async executeBulkAction() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    alert(data.message || @js(__('app.error_occurred')));
                }
            } catch (error) {
                console.error('Bulk action error:', error);
                alert(@js(__('app.operation_error')));
            } finally {
    ...
    window.Echo.private('church.{{ auth()->user()->church_id }}.people')
        .listen('.person.updated', function(e) {
            ...
            toast.innerHTML = '<svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>'
                + '<span>' + msg + '</span>';
            toast.onclick = function() { Livewire.navigate(window.location.href); };
