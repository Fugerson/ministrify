...
<div class="space-y-4 lg:space-y-6">
    ...
    <div class="lg:hidden flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Люди</h1>
            <p class="text-sm text-gray-500">{{ $people->total() }} {{ trans_choice('людина|людини|людей', $people->total()) }}</p>
        </div>
        ...
        <form method="GET" class="space-y-3 lg:space-y-0 lg:flex lg:items-center lg:gap-3">
            ...
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Пошук..."
                    class="w-full pl-11 pr-4 py-3 bg-gray-50 border-0 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary-500/20 transition-all">
            </div>

            <!-- Filters -->
            <div class="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0">
                <select name="tag" class="flex-shrink-0 px-4 py-3 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-primary-500/20 text-gray-700">
                ...
                <select name="ministry" class="flex-shrink-0 px-4 py-3 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-primary-500/20 text-gray-700">
                ...
                <button type="submit" class="flex-shrink-0 px-5 py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors">
                    Знайти
                </button>

                @if(request()->hasAny(['search', 'tag', 'ministry']))
                <a href="{{ route('people.index') }}" class="flex-shrink-0 px-4 py-3 text-gray-500 hover:text-gray-700">
                    Скинути
                </a>
                @endif
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-4">
        ...
        <a href="{{ route('people.show', $person) }}"
           class="group bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-lg hover:border-primary-100 hover:-translate-y-0.5 transition-all duration-200">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    @if($person->phone)
                    <p class="text-sm text-gray-500 truncate">{{ $person->phone }}</p>
                    @elseif($person->telegram_username)
                    <p class="text-sm text-primary-600 truncate">{{ $person->telegram_username }}</p>
                    @endif
                    ...
                    <div class="flex items-center gap-1 mt-2">
                        ...
                        <span class="text-xs text-gray-400 ml-1">+{{ $person->ministries->count() - 3 }}</span>
                        @endif
                    </div>
                    @endif
                </div>
            ...
            <div class="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-gray-50">
                ...
                <span class="text-xs text-gray-400">+{{ $person->tags->count() - 2 }}</span>
                @endif
            </div>
            @endif
        </a>
        ...
        <div class="col-span-full">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                <div class="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-gray-100 to-gray-50 rounded-2xl flex items-center justify-center">
    ...
    @admin
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Експорт / Імпорт</h3>
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="{{ route('people.export') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('people.import') }}" method="POST" enctype="multipart/form-data" class="flex-1 flex flex-col sm:flex-row gap-2">
                @csrf
                <label class="flex-1 flex items-center justify-center px-4 py-2.5 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-primary-300 hover:bg-primary-50/50 transition-colors">
                    <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                    </svg>
                    <span class="text-sm text-gray-600">Оберіть файл</span>
