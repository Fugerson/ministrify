<div x-data="{
    ...
    _t: {
        name_required: @js(__('app.name_required')),
        max_255_chars: @js(__('app.max_255_chars')),
        invalid_format: @js(__('app.invalid_format')),
        max_file_size: @js(__('app.max_file_size')),
    },
    ...
    validateField(field) {
        ...
        switch(field) {
            case 'name':
                if (!this.form.name.trim()) this.errors.name = this._t.name_required;
                else if (this.form.name.length > 255) this.errors.name = this._t.max_255_chars;
                break;
            case 'public_email':
                if (this.form.public_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.form.public_email))
                    this.errors.public_email = this._t.invalid_format;
                break;
        }
    }
}">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-3">
            ...
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-primary-600 dark:text-primary-400 mb-0.5">{{ __('app.onboarding_step_2') }}</p>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('app.church_profile') }}</h2>
            </div>
        </div>
    </div>
    ...
    <form class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-6 p-6 bg-gradient-to-br from-gray-50 to-gray-100/50 dark:from-slate-700/50 dark:to-slate-800/50 rounded-2xl border border-gray-200/50 dark:border-slate-600/50">
            ...
            <div class="flex-1">
                <h4 class="font-semibold text-gray-900 dark:text-white mb-2">{{ __('app.church_logo') }}</h4>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">{{ __('app.logo_format_hint') }}</p>
        ...
        <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                {{ __('app.church_name') }} <span class="text-red-500">*</span>
            </label>
            <input type="text" name="name"
                   ...
                   placeholder="{{ __('app.church_name_placeholder') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('app.city') }}</label>
                <div class="relative">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('app.address') }}</label>
                <div class="relative">
                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('app.phone') }}</label>
                <div class="relative">
                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
