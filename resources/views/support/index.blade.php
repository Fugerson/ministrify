...
<div x-data="supportKanban()" x-init="init()" class="h-[calc(100vh-200px)]">
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('app.support') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('app.support_your_requests') }}</p>
        </div>
            ...
            <div class="relative">
                ...
                <div class="p-3 pt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-800 dark:text-white text-sm">{{ __('app.support_open') }}</h3>
            ...
            <div class="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-350px)]">
                ...
                <template x-if="openTickets.length === 0">
                    <div class="text-center py-8 text-gray-400 dark:text-gray-500">
                        ...
                        <p class="text-sm">{{ __('app.support_no_open') }}</p>
                    </div>
                </template>
            </div>
        </div>
            ...
            <div class="relative">
                ...
                <div class="p-3 pt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-800 dark:text-white text-sm">{{ __('app.support_in_progress') }}</h3>
                        <span class="text-xs text-gray-400 dark:text-gray-500 bg-gray-200/50 dark:bg-gray-700/50 px-2 py-0.5 rounded-full font-medium"
            ...
            <div class="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-350px)]">
                ...
                <template x-if="inProgressTickets.length === 0">
                    <div class="text-center py-8 text-gray-400 dark:text-gray-500">
                        ...
                        <p class="text-sm">{{ __('app.support_no_in_progress') }}</p>
                    </div>
                </template>
            </div>
        </div>
            ...
            <div class="relative">
                ...
                <div class="p-3 pt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-800 dark:text-white text-sm">{{ __('app.support_resolved') }}</h3>
                        <span class="text-xs text-gray-400 dark:text-gray-500 bg-gray-200/50 dark:bg-gray-700/50 px-2 py-0.5 rounded-full font-medium"
            ...
            <div class="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-350px)]">
                ...
                <template x-if="resolvedTickets.length === 0">
                    <div class="text-center py-8 text-gray-400 dark:text-gray-500">
                        ...
                        <p class="text-sm">{{ __('app.support_no_resolved') }}</p>
                    </div>
                </template>
            </div>
        </div>
    ...
    <template x-if="allTickets.length === 0">
        <div class="absolute inset-0 flex items-center justify-center">
            <div class="text-center p-12">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('app.support_no_tickets') }}</h3>
                <p class="text-gray-500 dark:text-gray-400 mb-4">{{ __('app.support_empty_hint') }}</p>
...
function supportKanban() {
    const newCountLabel = @js(__('app.support_new_count'));
    ...
    return {
        ...
        renderTicketCard(ticket, isDone = false) {
            ...
            const unreadBadge = ticket.unread > 0
                ? `<span class="ml-auto px-1.5 py-0.5 text-[10px] font-bold bg-red-500 text-white rounded-full">${ticket.unread} ${newCountLabel}</span>`
                : '';
