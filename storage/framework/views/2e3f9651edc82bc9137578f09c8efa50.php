<!DOCTYPE html>
<html lang="uk" x-data="{ darkMode: localStorage.getItem('theme') === 'dark' }" :class="{ 'dark': darkMode }">
<head>
    <script>
        (function() {
        ...
        <div class="w-full max-w-5xl card overflow-hidden flex flex-col lg:flex-row">
            ...
            <aside class="w-full lg:w-80 p-6 lg:p-8 flex-shrink-0 relative bg-gray-50/80 dark:bg-slate-800/80">
                ...
                <nav class="space-y-1 mb-8">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stepKey => $stepConfig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button
                            @click="goToStep('<?php echo e($stepKey); ?>')"
                            :disabled="!canNavigateTo('<?php echo e($stepKey); ?>')"
                            class="step-item w-full flex items-center gap-4 px-4 py-3 rounded-xl text-left"
                            :class="{
                                'active': currentStep === '<?php echo e($stepKey); ?>',
                                'opacity-40 cursor-not-allowed': !canNavigateTo('<?php echo e($stepKey); ?>')
                            }"
                        >
                            <span class="step-number flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold"
                                  :class="{
                                      'active': currentStep === '<?php echo e($stepKey); ?>',
                                      'completed': stepsState['<?php echo e($stepKey); ?>']?.completed,
                                      'skipped': stepsState['<?php echo e($stepKey); ?>']?.skipped,
                                      'bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-gray-400': !stepsState['<?php echo e($stepKey); ?>']?.completed && !stepsState['<?php echo e($stepKey); ?>']?.skipped && currentStep !== '<?php echo e($stepKey); ?>'
                                  }">
                                <template x-if="stepsState['<?php echo e($stepKey); ?>']?.completed">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                                    </svg>
                                </template>
                                <template x-if="stepsState['<?php echo e($stepKey); ?>']?.skipped">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"/>
                                    </svg>
                                </template>
                                <template x-if="!stepsState['<?php echo e($stepKey); ?>']?.completed && !stepsState['<?php echo e($stepKey); ?>']?.skipped">
                                    <span><?php echo e($stepConfig['order']); ?></span>
                                </template>
                            </span>
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-sm"
                                   :class="{
                                       'theme-primary': currentStep === '<?php echo e($stepKey); ?>',
                                       'text-gray-900 dark:text-white': currentStep !== '<?php echo e($stepKey); ?>'
                                   }"><?php echo e($stepConfig['title']); ?></p>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$stepConfig['required']): ?>
                                    <p class="text-xs text-gray-400 dark:text-gray-500">опційно</p>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                            <svg x-show="currentStep === '<?php echo e($stepKey); ?>'" class="w-5 h-5 theme-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        function onboardingWizard() {
            return {
                ...
                steps: <?php echo json_encode($steps, 15, 512) ?>,
                stepsState: <?php echo json_encode($stepsState, 15, 512) ?>,
                progress: <?php echo json_encode($progress, 15, 512) ?>,
                loading: true,
                saving: false,
                serverError: null,
                successMessage: null,
...
<?php /**PATH /var/www/html/resources/views/onboarding/wizard.blade.php ENDPATH**/ ?>